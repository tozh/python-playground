...
class SendNotice():
    def __init__(self, path):
        ...
        self.task = 'notice'
        self.err_count = 0
        self.logger = Logger(self.path, self.task)
    ...
    def read_file(self, file, start_line=-1):
                ...
                if i > start_line:
                    if line.strip():
                        try:
                            ...
                            self.logger.error(file, err.message, line, i)
                            print line
                            continue
                        try:
                            # self.send_notice(uid)
                            self.logger.info(file, 'sent', uid, i)
        ...
        while idx < len(file_list):
            ...
            for file in file_list[idx:idx+5]:
                ...
                if 'process' not in file and not os.path.isdir(file_path):
                    ...
                    t = threading.Thread(target=self.read_file, args=(file, start_line))
                    threads.append(t)
