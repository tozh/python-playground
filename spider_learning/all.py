...
def get_followers(session, conn, cursor, user_token_id, page_start, page_end):
    count = 0
    count_should_be = 0
        ...
        try:
            ...
            for person in json_data["data"]:
                ...
                if f.set_follower(person["url_token"], authorization):
                    if send_to_db.send_f(cursor, f):
                        count += 1
                        if count % 100 == 0:
                            print("person count: " + str(count) + ". ")
                    conn.commit()
                else:
                    person_fail_info = "person=" + person["url_token"] + ". Should: " + str(count_should_be) + ", Real: " + str(count) + "."
                    write_log("person_failure_log", person_fail_info)

        except KeyError as ke:
            print(repr(ke))
            page_fail_info = "page=" + str(i) + ". Failure code: " + KeyError
...
if __name__ == '__main__':
    ...
    connect = send_to_db.connect_db(send_to_db.config)
    cursor = send_to_db.get_cursor(connect)

    print(get_followers(session, connect, cursor, 'luvddd' ,2880, 5000))
    cursor.close()
    connect.close()
